...
/// SQLite database manager for Seashell
public class DatabaseManager {
    internal var db: OpaquePointer?
    private let dbPath: String
    internal let queue = DispatchQueue(label: "com.seashell.database", attributes: .concurrent)
    ...
    private init() {
        ...
        let dbDirectory = homeDir.appendingPathComponent(".seashell")
        try? FileManager.default.createDirectory(at: dbDirectory, withIntermediateDirectories: true)
        
        self.dbPath = dbDirectory.appendingPathComponent("seashell_commands.db").path
        setupDatabase()
    }
    ...
    private func createTables() {
        ...
        // Wave Control Plane schema migration (non-destructive)
        migrateToWaveSchema()
    }

    /// Add Wave-specific columns and tables to the existing schema.
    /// Uses ALTER TABLE with IF NOT EXISTS guards so it's safe to call repeatedly.
    private func migrateToWaveSchema() {
        // Add Wave workspace/block tracking columns to commands table.
        // SQLite ignores these if the column already exists (we catch the error).
        let waveColumnsSQL = [
            "ALTER TABLE commands ADD COLUMN wave_workspace_id TEXT",
            "ALTER TABLE commands ADD COLUMN wave_block_id TEXT",
        ]

        for sql in waveColumnsSQL {
        ...
        // Wave profiles table — stores Wave workspace preset definitions
        let createWaveProfilesTable = """
            CREATE TABLE IF NOT EXISTS wave_profiles (
                id TEXT PRIMARY KEY,
                name TEXT NOT NULL UNIQUE,
                workspace_id TEXT,
                theme TEXT,
        ...
        """
        if !execute(createWaveProfilesTable) {
            print("[DatabaseManager] Failed to create wave_profiles table")
        }

        print("[DatabaseManager] Wave schema migration complete")
    }
