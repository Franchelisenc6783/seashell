...
private func newTabAppleScript(for terminal: TerminalConfig.TerminalType) -> String {
    ...
    case .wave:
        // Wave Terminal — activate and let the user pick a block
        return """
...
/// Generate AppleScript to send command to the CURRENT active tab (Alacritty/Wave — no native tab targeting)
...
private func closeTabAppleScript(for terminal: TerminalConfig.TerminalType) -> String {
    ...
    case .wave:
        // Wave uses its own tab model; closing via Cmd+W
        return """
        tell application "Wave" to activate
        delay 0.3
        tell application "System Events"
...
func handleSendToSession(params: CallTool.Parameters, logger: Logger) async -> CallTool.Result {
    ...
    case .alacritty, .wave:
        // These terminals don't support tab targeting via AppleScript.
        // Best effort: activate terminal and type into the current active tab.
